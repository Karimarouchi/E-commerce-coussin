...
public class CategoryService {
    ...
    // ── Admin: Toggle statut only (ne touche pas à l'image ni aux autres champs) ──
    @Transactional
    public CategoryResponse toggleStatut(Long id) {
        Category category = findOrThrow(id);
        String current = category.getStatut() == null ? "actif" : category.getStatut();
        category.setStatut("actif".equalsIgnoreCase(current) ? "désactivé" : "actif");
        return mapToResponse(categoryRepository.save(category));
    }

    // ── Admin: Toggle vedette only ─────────────────────────────────────
    @Transactional
    public CategoryResponse toggleVedette(Long id) {
        Category category = findOrThrow(id);
        category.setVedette(!category.isVedette());
        return mapToResponse(categoryRepository.save(category));
    }
    ...
    private CategoryResponse mapToResponse(Category category) {
        return CategoryResponse.builder()
                ...
                .productCount(productRepository.countByCategoryIdOrParentId(category.getId()))
